...
//var serverURL = 'http://192.168.88.88:5245/Prodigy/CommonBuildJSON';
//var serverURL = 'http://192.168.88.33:8080/MBK/CommonBuildJSON';
//var serverURL = 'http://192.168.88.103:8080/MBK/CommonBuildJSON';
var serverURL = 'http://192.168.88.88:5245/Prodigy/CommonBuildJSON';
//var serverURL = 'http://192.168.88.146:8080/MBK/CommonBuildJSON';
//var serverURL = 'http://prodigy.t-bop.com:5245/Prodigy/CommonBuildJSON';
/******************************************
// Utility
******************************************/
localStorage.setItem("DEVTEST", true)
...
function getPullCRED(){
	...
	var userid = $("#syncuserid").val();
	var userpw = $("#syncpassword").val();
	var managerid = $("#syncmgrid").val();
	sessionStorage.setItem("CASHIER_ID",managerid);
	var managerpw = $("#mgrsyncpassword").val();

	return [userid,userpw,managerid,managerpw];
}

function getPushCRED(){
 
	var userid = $("#syncuserid_push").val();
	var userpw = $("#syncpassword_push").val();
	var managerid = $("#syncmgrid_push").val();
	var managerpw = $("#mgrsyncpassword_push").val();
...
function getResetCRED(){
	...
	var userpw = $("#resetpassword").val();
	var managerid = $("#resetmgrid").val();
	var managerpw = $("#mgrresetpassword").val();

	return [userid,userpw,managerid,managerpw];
} 
localStorage.setItem("SYNC_SERVER",serverURL);
$('#setip').val(serverURL);
...
function Sync(){
	...
	******************************************/
	...
	this.checkVersion = function(){
		...
		AjaxSyncCheckVersion = $.ajax({ //!Note: Probably refactor all ajax calls so we don't have multiples of the same declaration
			url: serverURL,
			type: "POST",
			async: true,
			cache: false,
...
endSync = function(text){
	...
	if(syncbox) syncbox.html(text);
	var quit = $('<button>Close</button>');
	quit.on('click', function(){closeBox();});
	quit.hide().appendTo(syncbox).fadeIn();
	syncbox.trigger("create");
};
...
function init(){
	...
	function pushData(){
		//Pushing Data
		//Transfer Push CRED to original Keys to make it work
		$("#syncuserid").val($("#syncuserid_push").val());
		$("#syncpassword").val($("#syncpassword_push").val());
		$("#syncmgrid").val( $("#syncmgrid_push").val());
		$("#mgrsyncpassword").val($("#mgrsyncpassword_push").val());
	...
	$("#resetprint").on("click", function(){
		...
		function resetClientRepay(){
			...
			var cmd1 =  "   update T_CLIENT_REPAY_SCHEDULE set CRS_TOTALPAID = '0.00' , CRS_ATTENDED = null , CRS_SMS_STATUS= null , ";
				...
				cmd1 += "   crs_flexi_exp_19    = '0.00',   crs_flexi_exp_20   = '0.00',    ";//FLEXI EXP  
	...
	if(true){
		...
		setTimeout(function(){
			...
			var cmd =   ' SELECT CRS_FLAG, CRS_ATTENDED,CRS_DATE,CRS_PK,'+
					...
					' WHERE CRS_CLL_PK = CLL_PK AND CRS_FLAG ="C" AND ( CRS_PK IN ('+localStorage.getItem('TODAYS_CRS_PKS')+') AND (  CRS_DATE="'+date+'" AND CRS_ATTENDED != "") )  ';//CLT_STATUS!=8 AND
			...
			myDB.execute(cmd, function(results){
				...
				if(results.length > 0){
					...
					$.each(results, function(i, record){
						//ttldis += record.CRS_DISBURSEMNT_AMT;
						ttlsaved += parseFloat(record.savings_total_act);
						ttlsave += parseFloat(record.savings_total_exp);
						// ttlcoled += parseFloat(record.CRS_BALANCE_CAPITAL);
						// ttlcol += parseFloat(record.CRS_COLLECTION_AMT);
					});
			...
			myDB.execute(cmd, function(results){
				...
				if(results.length > 0 && results[0].CRS_FLAG !== null){
					...
					$.each(results, function(i, record){
						...
						var crf = (100-record.CLL_CRF_PERCENTAGE)/100;
						////console.log(crf);
						////console.log(record.savings_total_act);
						var disamt = record.savings_total_exp  * parseFloat(crf);
		...
		anotherdate = anotherdate.getFullYear()+"-"+("0"+(anotherdate.getMonth()+1)).slice(-2)+"-"+("0"+anotherdate.getDate()).slice(-2);
			...
			myDB.execute(cmd,function(prds){
				...
				var deposit =   " SELECT *, DATE(CLT_TRANING_END_DATE) as TRAINING_END "+
							" FROM T_CLIENT,T_CLIENT_LOAN,T_LOAN_TYPE,T_LOAN_SAVING_PRD_MAPPING "+
							" WHERE CLT_PK = CLL_CLT_PK AND CLL_STATUS IN (3,26,57,64) AND CLL_LTY_PK = LTY_PK AND LTY_PK = LSM_LTY_PK "+
...
function getTodaysCRS() {
	...
	myDB.execute("SELECT MIN(CRS_PK) as CRS_PK FROM T_CLIENT_REPAY_SCHEDULE LEFT JOIN T_CLIENT_LOAN ON (CRS_CLL_PK = CLL_PK) WHERE CRS_ATTENDED = '' AND CLL_STATUS != 9 AND CRS_LOAN_SAVING_FLAG='L' GROUP BY CRS_CLL_PK",function(res){
		console.log(res);
		var crss = "";
...
function updateCRS(){
	...
	if(turnon){
		// myDB.execute("UPDATE T_CLIENT_REPAY_SCHEDULE SET CRS_LOAN_SAVING_FLAG='H' WHERE CRS_LOAN_SAVING_FLAG='S'",function(rest){
		//     console.log("done");
		// });
		var username = $("#userid").val();
		var password = $("#password").val();
		...
		myDB.execute("SELECT MAX(CRS_PK) as maxcrs, USER_PK FROM T_CLIENT_REPAY_SCHEDULE,T_USER,T_COMPANY,T_BRANCH WHERE USER_ID='"+username+"' AND USER_PASSWORD='"+password+"' AND USER_BRC_PK=BRC_PK AND BRC_CMP_PK=CMP_PK AND USER_ROLE = 4 LIMIT 1",function(data){
            ...
            myDB.execute("SELECT * FROM T_PRODUCT_MASTER_DETAILS",function(data){
                ...
                myDB.execute(query,function(clients){
                    ...
                    // 	if(client.CLT_PK == 2426 || client.CLT_PK == 5709 || client.CLT_PK == 9524){
                    // 		console.log("IM HERE "+client.CLT_PK);
                    // 	}
