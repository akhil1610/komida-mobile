...
// window.serverURL = 'http://101.127.229.140:5245/MBK/CommonBuildJSON';
//var window.serverURL = 'http://192.168.88.224:9080/MBK/CommonBuildJSON';
//var window.serverURL = 'http://192.168.88.146:8080/MBK/CommonBuildJSON';
//var window.serverURL = 'http://prodigy.t-bop.com:5245/Prodigy/CommonBuildJSON';
//var window.serverURL   = 'http://komidauat.t-bop.com/Prodigy/CommonBuildJSON';
window.SERVERS = {
	'http://192.168.88.235:8080/MBK/CommonBuildJSON': 'Backup',
...
myDB.execute("SELECT USER_ID FROM T_USER WHERE USER_ROLE=4 ",function(res){
    console.log(res);
    if(res.length > 0) {
        var USER_ID = res[0].USER_ID;
        $('#loggedinfoid').html(USER_ID);
        $('#fouserid').val(USER_ID);
    }
});
...
/******************************************
// Load file manager only when it's ready
******************************************/
...
$('#logoutbutton').on('click', function(){ 
    ...
    .then( result => {
        if(result){
            fologoutSuccess()
            console.log('fologoutsuccess');
            return mgrlogout();
        }
    })
    .then( result => {
        if(result){
            console.log('mgrlogoutsuccess');
            mgrlogoutSuccess();
            setTimeout(() => {
...
function fologout(){ 
    ...
    var p = new Promise( (res, rej) => {
        $.ajax({
            ...
            cache: false,
            type: "POST",
            data: "strMode=FoLogout&cred="+JSON.stringify(obj),
            crossDomain: true,
            processData: false,
...
function mgrlogout(){ 
    ...
    var p = new Promise((res,rej) => {
        $.ajax({
            ...
            cache: false,
            type: "POST",
            data: "strMode=MgrLogout&cred="+JSON.stringify(obj),
            crossDomain: true,
            processData: false,
            success: function(result){ 
                swal.close();
...
function ResponseFOSecond(type){
	...
	myDB.execute("SELECT USER_PK FROM T_USER WHERE USER_ROLE=4 ",function(res){
        ...
        swal.queue([{
            title: i18n.t("messages.ResetingDevice"),  
            onOpen: () => {
                swal.showLoading();
            },
			...
			success: function(result){
				...
				if(result=="Success"){
					...
					if(type=='Reset' || type == 'Push') {
						//endSync(i18n.t("messages.FOResetSuccess"),'success');
						myDB.reset();
						setTimeout(function(){ 
					...
					if(type=='RESET')endSync(i18n.t("messages.FOResetFail"), 'error');
					return true;
				}
			},
			error: function(jqXHR,textStatus,errorThrown){
				...
				if(type == 'Pull'){
					updateCRS();
				} else if(type=='reset')endSync(i18n.t("messages.FOResetFail")+jqXHR.responseText, 'error');
...
function Sync(){
	...
	******************************************/
	this.syncError = function(){
		...
		syncmgr.endsync(i18n.t("messages.SyncError"), 'error');
	...
	var PSYNC = {
        ...
        sendGroup:false,
        updatePpiData: false,
        updateLoanMonitoring: false,
        updateFailedDisbursement: false,
        clientExit: false,
        clientExitAccounts: false,
	};
	...
	syncmgr.beginSync = function(syncmode){
		...
		syncmgr.orderSync.syncfn_arr = [ //functions declared below
            ...
            syncmgr.sendGroup,
            syncmgr.clientExit,
            syncmgr.clientExitAccounts,
            syncmgr.updatePpiData,
            syncmgr.updateLoanMonitoring,
            syncmgr.updateFailedDisbursement, 
		];
		else //pull request
	...
	this.checkVersion = function(){
		...
		$.ajax({ //!Note: Probably refactor all ajax calls so we don't have multiples of the same declaration
			...
			error: function(jqXHR,textStatus,errorThrown){
				...
				if(err !== undefined){
					syncmgr.endsync(i18n.t("messages.VersionCheckFail") + err.Message, 'error');
				}
	...
	******************************************/
	this.checkRequirements = function(){
		...
		myDB.execute("SELECT CLL_STATUS FROM T_CLIENT,T_CLIENT_LOAN,T_LOAN_TYPE WHERE CLT_PK = CLL_CLT_PK AND CLT_SIGNATURE = '' AND ( CLL_STATUS = 12 OR CLL_FORM_DISTRIBUTION_DATE = '"+date+"' ) GROUP BY CLT_PK LIMIT 1", function(loanrec){
			if(loanrec.length>0){
				syncmgr.endsync(i18n.t("messages.SignByApplicantStatusNotSetForUser"), 'error');
				return false;
			}
			promise.resolve();
		});
	...
	******************************************/
	this.checkRepayments = function(){
		...
		//myDB.execute("SELECT CLL_STATUS FROM T_CLIENT_LOAN,T_LOAN_TYPE,T_CLIENT_REPAY_SCHEDULE WHERE ( CLL_LTY_PK = LTY_PK AND CRS_CLT_PK = CLL_CLT_PK AND CRS_DATE = '"+date+"' AND ( (CRS_COLLECTION_WEEK_NO >= LTY_FIRST_REVIEW_WEEK AND CLL_STATUS = 17) ) OR  (CRS_COLLECTION_WEEK_NO >= LTY_SECOND_REVIEW_WEEK AND CLL_STATUS = 19) ) OR ( CLL_STATUS = 10 and CLL_FORM_DISTRIBUTION_DATE = '"+date+"') LIMIT 1", function(loanrec){
		var cmd = "SELECT CRS_CLT_PK FROM T_CLIENT_REPAY_SCHEDULE WHERE (CRS_LOAN_SAVING_FLAG='S' OR CRS_FLAG='D' ) AND CRS_DATE='"+date+"' AND (CRS_RECP_GROUP_STATUS NOT IN  ('Y','N') AND CRS_SMS_GROUP_STATUS NOT IN  ('Y','N') AND CRS_RECP_STATUS NOT IN  ('Y','N') AND CRS_SMS_STATUS NOT IN  ('Y','N') ) AND CRS_FO="+sessionStorage.getItem("USER_PK");
        console.log(cmd);
        console.log('repayment check');
		myDB.execute(cmd, function(loanrec){
            console.log(loanrec);
			if(loanrec.length>0){
				syncmgr.endsync(i18n.t("messages.PendingCollections"), 'error');
				return false;
			}
			promise.resolve();
		});
	...
	this.checkUnassignedClients = function(){
		...
		myDB.execute("SELECT CLT_PK FROM T_CLIENT WHERE CLT_MOB_NEW=1 and CLT_GROUP_ID='' LIMIT 1", function(results){
			...
			if(results.length==1){ //at least 1 user with no group
				...
				syncmgr.endsync(i18n.t("messages.ClientUnassignedToGroup"), 'error');
				return false;
			}
	...
	******************************************/
	this.syncLoans = function(){ //begin syncing
		...
		myDB.T_CLIENT.get("CLT_MOB_NEW=1", function(results){//get all new records
            ...
            function checkPPI(CLTNO, reqObj) {
                console.log("@@@@@@@@@@@@@@@@@@@@@ CHECKING PPI ");
                myDB.T_PPI_FORM.get("PPIF_CLT_PK="+CLTNO, function(results) {
                    var PPIF_ARR = [];
                    if(results.length === 0) {
                        reqObj.PPIFORM = [];
                        reqObj.PPIANS = []
                        ...
                        PPIF_ARR = [
                            PPIF.PPIF_PK,
                            PPIF.PPIF_TOTAL_SCORE,
                            PPIF.PPIF_MEMBER_PK,
                            PPIF.PPIF_FO_PK,
                            PPIF.PPIF_WITNESS_PK,
                            PPIF.PPIF_MANAGER_PK,
                            PPIF.PPIF_CENTER_LEADER_PK,
                            PPIF.PPIF_GROUP_LEADER_PK,
                            PPIF.PPIF_FAMILY_MEMBER,
                            PPIF.PPIF_FAMILY_MEMBER_NAME, 
                            PPIF.PPIF_CREATED_DATE,
                            PPIF.PPIF_FAMILY_SIGN
                        ];
                        reqObj.PPIFORM = PPIF_ARR;
                        
                        myDB.T_PPI_ANSWER.get("PPIA_PPIF_PK="+PPIF.PPIF_PK, function( res) {
                            var PPIA_ARR = [];
                            if(res.length === 0) {
                                reqObj.PPIANS = []
                            } else {
                                ...
                                res.forEach(function(val, ind){
                                    ...
                                    var PPIA_OBJ = [
                                        ...
                                        val.PPIA_ANSWER,
                                        val.PPIA_SCORE,
                                        val.PPIA_CREATED_DATE
                                    ];
            
                                    PPIA_OBJ = nullToEmpty(PPIA_OBJ);
                                    PPIA_ARR.push(PPIA_OBJ);
            
                                    var ppia_cnt = parseInt(ind) + 1;
                                    if( res.length == ppia_cnt ){
                                        reqObj.PPIANS = PPIA_ARR;
                                        checkBorrowed(CLTNO,reqObj);
                                    }
                                });
                            }
                    ...
                    success: function(result){
                        if(result=="Success"){
                            ...
                            myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                                ...
                                tx.executeSql("DELETE FROM T_PPI_FORM WHERE PPIF_CLT_PK="+CLTNO);
                            }, function(err){
                            ...
                            if( result == 'Invalid UserId / Password'){
                                syncmgr.endsync(i18n.t("messages.CredsMismatch"), 'error');
                            } else if (result == 'FO cannot Push data again.'){
                                syncmgr.endsync(i18n.t("messages.FOCannotPushAgain"), 'error');
                            } else {
                                syncmgr.endsync(i18n.t("messages.ClientSyncFail"), 'error');
                            }
                            return true;
                        }
                        report();
			...
			function report(){ //all clients and loans synced, tally results
				if((success+failed)==total){ //all requests completed
					if(success==total){
						...
						syncmgr.endsync(i18n.t("messages.SomeSyncRequestFail"), 'error');
						return false;
					}
				}
			}
	...
	******************************************/
	this.syncNewLoans = function(){ //begin syncing
		...
		myDB.execute("SELECT T_CLIENT.* FROM T_CLIENT,T_CLIENT_LOAN WHERE CLL_CLT_PK = CLT_PK AND CLL_MOB_NEW=1 ",function(results){
            ...
            function checkPPI(CLTNO, reqObj) {
                console.log("@@@@@@@@@@@@@@@@@@@@@ CHECKING PPI ");
                myDB.T_PPI_FORM.get("PPIF_CHKNEW = 2 AND PPIF_CLT_PK="+CLTNO, function(results) {
                    var PPIF_ARR = [];
                    if(results.length === 0) {
                        reqObj.PPIFORM = [];
                        reqObj.PPIANS = []
					...
					success: function(result){
						if(result=="Success"){
							...
							myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                                ...
                                tx.executeSql("DELETE FROM T_CLIENT_PRODUCT_MAPPING WHERE CPM_CHKNEW=1 AND CPM_CLT_PK="+CLTNO);
                                tx.executeSql("DELETE FROM T_PPI_FORM WHERE PPIF_CHKNEW=2 AND PPIF_CLT_PK="+CLTNO);
							}, function(err){
	...
	******************************************/
	...
	this.updateTraining = function(){
        ...
        var TRAINING_ARR = []; 
		var cmd = "SELECT CLT_PK, CLT_STATUS, CLT_TRAINING_ATTENDANCE FROM T_CLIENT LEFT JOIN T_CLIENT_TRAINING_TMP ON (CTT_CLT_PK = CLT_PK)  WHERE CTT_STATUS = 1 AND CLT_STATUS in (4,6,26,64,65,66,67) AND DATE(CLT_TRANING_END_DATE) > '"+anotherdate+"' AND DATE(CLT_TRANING_START_DATE) <='"+anotherdate+"' AND CLT_FO_FOR_TRAINING= "+localStorage.getItem("USER_PK");
		console.log(cmd);
	...
	this.updateTest = function(){
        ...
        myDB.execute("SELECT CLL_PK, CLL_CLT_PK, CLL_STATUS, CLL_MANAGER_PK, CLL_WITNESS1_PK, CLL_WITNESS2_PK, CLL_WITNESS3_PK, CLL_WITNESS4_PK, CLL_TEST_PLACE, CLL_ORIGINAL_LOAN, CLL_LPU_PK, CLL_TOTAL_LOAN_WEEKS, CLL_LOAN_INTEREST, CLL_GRACE_PERIOD_WEEKS, CLL_CENTER_LEAD_PK, CLL_GROUP_LEAD_PK   FROM T_CLIENT_LOAN, T_CLIENT WHERE CLL_CLT_PK = CLT_PK AND (DATE(CLT_TRANING_END_DATE) ='"+anotherdate+"' OR CLL_STATUS = 64 )  ", function(loans){
			////console.log(loans);
			if(loans.length === 0){
			...
			$.each(loans, function(i,loan){ 
                ...
                if(loan.CLL_GROUP_LEAD_PK != '') grp_pk = loan.CLL_GROUP_LEAD_PK;
                if(loan.CLL_CENTER_LEAD_PK != '') ctr_pk = loan.CLL_CENTER_LEAD_PK;
				...
				var loan_key =  [
							...
							wit4_pk,
                            loan.CLL_TEST_PLACE,
                            loan.CLL_ORIGINAL_LOAN, 
                            loan.CLL_LPU_PK, 
                            loan.CLL_TOTAL_LOAN_WEEKS, 
                            loan.CLL_LOAN_INTEREST,
                            loan.CLL_GRACE_PERIOD_WEEKS,
                            ctr_pk,
                            grp_pk
						];
		...
		function loadProducts(loans){
			...
			$.each(loans,function(l,loan){
				...
				//     return false;
				// }
				myDB.execute("SELECT * FROM T_CLIENT_PRODUCT_MAPPING,T_PRODUCT_MASTER WHERE CPM_PRM_PK = PRM_PK AND CPM_CLL_PK="+loan[0],function(cpm){
					flags.CPM = true;
					////console.log(cpm);
					if(cpm.length === 0){
					...
					$.each(cpm, function(c,prd){
                        ...
                        var isNew = prd.CPM_CHKNEW == 1 ? 'Y' : 'N';
	...
	******************************************/
	this.pushClientPrd = function(){
		...
		var clientnewprd = {};
		clientnewprd.DATA = [];
		var cpmcmd = 'SELECT CPM.*, PRM.PRM_IS_MANAGER_APPROVAL, CLT_STATUS FROM T_CLIENT_PRODUCT_MAPPING as CPM LEFT JOIN T_PRODUCT_MASTER as PRM ON (CPM.CPM_PRM_PK = PRM.PRM_PK) LEFT JOIN T_CLIENT ON (CPM_CLT_PK = CLT_PK) WHERE CPM.CPM_CHKNEW = 2';
		...
		myDB.execute(cpmcmd, function(datares){
			...
			$.each(datares, function(id,reval){
                ...
                newprd.push(reval.PRM_IS_MANAGER_APPROVAL);
                newprd.push(reval.CLT_STATUS);
	...
	this.sendEval = function(){
		...
		myDB.T_CLIENT_EVALUATION.get("CLE_MOB_NEW=1", function(results){
			...
			$.each(results,function(id,evalu){
                ...
                newevalObj.push(evalu.CLE_COMPLIANCE);
                newevalObj.push(evalu.CLE_COMPLIANCE_OTHERS);
                newevalObj.push(evalu.CLE_OTHER_INSTITUITIONAL_FINANCING);
                newevalObj.push(evalu.CLE_OTHER_INSTITUITIONAL_NAME);
                newevalObj.push(evalu.CLE_AMOUNT_OF_FINANCING);
                newevalObj.push(evalu.CLE_PAID_OFF);
                newevalObj.push(evalu.CLE_FO_NOTE); 
                newevalObj.push(evalu.CLE_REPAY_PER_WEEK);
                newevalObj.push(evalu.CLE_ASSET_CONDITION);
                newevalObj.push(evalu.CLE_TOTAL_ASSET);
                newevalObj.push(evalu.CLE_HUSBAND_INCOME);
                newevalObj.push(evalu.CLE_WIFE_INCOME);
                newevalObj.push(evalu.CLE_OTHER_INCOME);
                newevalObj.push(evalu.CLE_HOUSEHOLD_EXPENSES);
                newevalObj.push(evalu.CLE_EDUCATION_EXPENSES);
                newevalObj.push(evalu.CLE_EXTRA_EXPENSES);
					...
					myDB.T_CLIENT_EVAL_ASSET_LIST.get('CEAL_CLE_PK='+evalpk, function(assetlists){
						...
						$.each(assetlists,function(a, assetlist){
							...
							var asset = [
                                ...
                                assetlist.CEAL_ASSET_CONDITION,
							];
	...
	******************************************/
    ...
    /******************************************
	// Update Loan Monitoring
	******************************************/
    this.updateLoanMonitoring = function() {
        var promise = new $.Deferred();
        syncmgr.synclog(i18n.t("messages.UpdatingLoanMonitoring")); 
        reqObj =  { 
            LOANMON: []
        }
        myDB.T_LOAN_MONITORING.get("", function(results){
            if(results.length > 0)  {
                results.forEach(res => {
                    var LM = res;
                    var LM_ARR = [
                        ...
                        LM.LM_COMPLIANCE,
                        LM.LM_OTHERS,
                        LM.LM_CENTER_LEADER_PK,
                        LM.LM_CLIENT_SIGN_PK,
                        LM.LM_CREATED_TIMESTAMP,
                    ];
                    reqObj.LOANMON.push(LM_ARR);
                });
                checkfinal(reqObj);
            } else {
                PSYNC.updateLoanMonitoring = true;
                promise.resolve();
                return false;
            }
        });

        function checkfinal(reqObj){ 
            reqObj.CRED = getPushCRED(); 
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: "strMode=saveLoanMon&data="+JSON.stringify(reqObj),
                crossDomain: true,
                processData: false,
                success: function(result){
                    if(result=="Success"){  
                        myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                            tx.executeSql("DELETE FROM T_LOAN_MONITORING"); 
                        }, function(err){
                            syncmgr.endsync(i18n.t("messages.LoanMonitoringSyncFail"), 'error');
                            return false;
                        }, function(suc){
                            syncmgr.synclog(i18n.t("messages.LoanMonitoringSyncSuccess"));
                            PSYNC.updateLoanMonitoring = true;
                            promise.resolve();
                        });
                    }else{  
                ...
                error: function(jqXHR,textStatus,errorThrown){ 
                    syncmgr.endsync(i18n.t("messages.LoanMonitoringSyncFail")+jqXHR.responseText);
                    return false;
                }
            }); 
        } 
    ...
    /******************************************
	// Update Failed Disbursement
	******************************************/
    this.updateFailedDisbursement = function() {
        var promise = new $.Deferred();
        syncmgr.synclog(i18n.t("messages.UpdatingDisbursementRescheduling")); 
        reqObj =  { 
            FAILDIS: []
        }
        myDB.execute("SELECT * FROM T_FAIL_DISBURSE_LOAN", function(results){
            if(results.length > 0)  {
                results.forEach(res => {
                    var FDL = res;
                    var FDL_ARR = [
                        FDL.FDL_PK,
                        FDL.FDL_OLD_CLL_PK,
                        FDL.FDL_CLL_ACTUAL_LOAN,
                        FDL.FDL_CLL_LOAN_WEEKS,
                        FDL.FDL_CLL_DISBURSEMENT_DATE,
                        FDL.FDL_CREATED_BY,
                        FDL.FDL_CREATED_DATE,
                    ];
                    reqObj.FAILDIS.push(FDL_ARR);
                });
                checkfinal(reqObj);
            } else {
                PSYNC.updateFailedDisbursement = true;
                promise.resolve();
                return false;
            }
        });
        function checkfinal(reqObj){ 
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: "strMode=failDisb&data="+JSON.stringify(reqObj),
                crossDomain: true,
                processData: false,
                success: function(result){
                    if(result=="Success"){  
                        myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                            tx.executeSql("DELETE FROM T_FAIL_DISBURSE_LOAN"); 
                        }, function(err){
                            syncmgr.endsync(i18n.t("messages.DisbursementReschedulingFail"), 'error');
                            return false;
                        }, function(suc){
                            syncmgr.synclog(i18n.t("messages.DisbursementReschedulingSuccess"));
                            PSYNC.updateFailedDisbursement = true;
                            promise.resolve();
                        });
                    }else{  
                ...
                error: function(jqXHR,textStatus,errorThrown){ 
                    syncmgr.endsync(i18n.t("messages.DisbursementReschedulingFail")+jqXHR.responseText);
                    return false;
                }
            }); 
        } 
	...
	// Update Client Exit
	******************************************/
    this.clientExit = function() {
        var promise = new $.Deferred();
        syncmgr.synclog(i18n.t("messages.UpdatingClientExit")); 
        reqObj =  { 
            CLIENTEXIT: []
        }
        myDB.execute("SELECT * FROM T_CLIENT_EXIT_FORM", function(results){
            if(results.length > 0)  {
                results.forEach(res => {
                    var CEF = res;
                    var CEF_ARR = [
                        CEF.CEF_CLT_PK,
                        CEF.CEF_BRANCH_PK,
                        CEF.CEF_DUE_PRINCIPAL,
                        CEF.CEF_DUE_INTEREST,
                        CEF.CEF_PRINCIPAL_NOT_DUE,
                        CEF.CEF_INTEREST_NOT_DUE,
                        CEF.CEF_INTEREST_BALANCE,
                        CEF.CEF_DISBUSEMENT_AMOUNT,
                        'null', // CEF.CEF_ISWO,
                        CEF.CEF_STATUS,
                        CEF.CEF_CREATED_BY,
                        CEF.CEF_CREATED_DATE, //11
                        CEF.CEF_APPROVED_BY_EDC,
                        CEF.CEF_OTHER_INSTITUTION_LOANS,
                        CEF.CEF_SICK,
                        CEF.CEF_NO_HUSBAND_PERMISSION,
                        CEF.CEF_MOVING_HOUSE,
                        CEF.CEF_BANKRUPT,
                        CEF.CEF_LOSS,
                        CEF.CEF_LARGER_LOANS,
                        CEF.CEF_NOT_HAPPY_PRODUCT, //20
                        CEF.CEF_NOT_HAPPY_SERVICE,
                        CEF.CEF_NOT_HAPPY_REGULATIONS,
                        CEF.CEF_REQUEST_EXIT,
                        CEF.CEF_BROKE_UP,
                        CEF.CEF_MEMBER_CONFLICT,
                        CEF.CEF_CHAIRPERSON_CONFLICT,
                        CEF.CEF_UNCOMFORTABLE,
                        CEF.CEF_NOT_PRESENT,
                        CEF.CEF_DONT_NEED_LOAN,
                        CEF.CEF_NEED_MONEY, //30
                        CEF.CEF_ATTENDANCE_PROBLEM,
                        CEF.CEF_INSTALLMENT_PROBLEMS,
                        CEF.CEF_MOVE_ADDRESS,
                        CEF.CEF_EMPLOYMENT_OUT_OF_STATE,
                        CEF.CEF_DIE, // 35
                        CEF.CEF_FO_SIGN,
                        CEF.CEF_CLIENT_SIGN,
                        CEF.CEF_REASON,
                    ];
                    reqObj.CLIENTEXIT.push(CEF_ARR);
                });
                checkfinal(reqObj);
            } else {
                PSYNC.clientExit = true;
                promise.resolve();
                return false;
            }
        });
        function checkfinal(reqObj){ 
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: "strMode=clientExit&data="+JSON.stringify(reqObj),
                crossDomain: true,
                processData: false,
                success: function(result){
                    if(result=="Success"){  
                        myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                            tx.executeSql("DELETE FROM T_CLIENT_EXIT_FORM"); 
                        }, function(err){
                            syncmgr.endsync(i18n.t("messages.ClientExitFail"), 'error');
                            return false;
                        }, function(suc){
                            syncmgr.synclog(i18n.t("messages.ClientExitSuccess"));
                            PSYNC.clientExit = true;
                            promise.resolve();
                        });
                    }else{  
                ...
                error: function(jqXHR,textStatus,errorThrown){ 
                    syncmgr.endsync(i18n.t("messages.ClientExitFail")+jqXHR.responseText);
                    return false;
                }
            }); 
        } 
    ...
    /******************************************
	// Update Client Exit Accounts
	******************************************/
    this.clientExitAccounts = function() {
        var promise = new $.Deferred();
        syncmgr.synclog(i18n.t("messages.UpdatingClientExit")); 
        reqObj =  { 
            CLIENTCLOSE: []
        }
        myDB.execute("SELECT * FROM T_CLIENT_CLOSE_PRODUCTS", function(results){
            if(results.length > 0)  {
                results.forEach(res => {
                    var CCP = res;
                    var CCP_ARR = [
                        CCP.CCP_CLT_PK,
                        CCP.CCP_ACCOUNT_NUMBER,
                        CCP.CCP_BALANCE_AMT,
                        CCP.CCP_WITHDRAWAL_AMT,
                        CCP.CCP_CLOSING_AMT,
                        CCP.CCP_STATUS,
                        CCP.CCP_CREATED_BY,
                        CCP.CCP_CREATED_DATE,
                    ];
                    reqObj.CLIENTCLOSE.push(CCP_ARR);
                });
                checkfinal(reqObj);
            } else {
                PSYNC.clientExitAccounts = true;
                promise.resolve();
                return false;
            }
        });
        function checkfinal(reqObj){ 
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: "strMode=clientClosePrd&data="+JSON.stringify(reqObj),
                crossDomain: true,
                processData: false,
                success: function(result){
                    if(result=="Success"){  
                        myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                            tx.executeSql("DELETE FROM T_CLIENT_CLOSE_PRODUCTS"); 
                        }, function(err){
                            syncmgr.endsync(i18n.t("messages.ClientExitFail"), 'error');
                            return false;
                        }, function(suc){
	...
	// Update New PPI
	******************************************/
	this.updatePpiData = function(){

		var promise = new $.Deferred();
		// syncmgr.synclog("Updating group leaders..");
        syncmgr.synclog(i18n.t("messages.UpdatingPPI")); 
        reqObj =  {
            PPIFORM: [],
            PPIANS: []
        }
        myDB.T_PPI_FORM.get("PPIF_CHKNEW = 2", function(results) { 
            var PPIF_PK_ARR = [];
            var PPIF_PK_STR = '';
            if(results.length > 0)  {
                results.forEach(function(PPIF, r) { 
                    var PPIF_ARR = [
                        PPIF.PPIF_PK,
                        PPIF.PPIF_TOTAL_SCORE,
                        PPIF.PPIF_MEMBER_PK,
                    ...
                    reqObj.PPIFORM.push(PPIF_ARR);
                    if(PPIF_PK_ARR.indexOf(PPIF.PPIF_PK) == -1) {
                        PPIF_PK_ARR.push(PPIF.PPIF_PK);
                        if(PPIF_PK_STR !== '') PPIF_PK_STR += ",";
                        PPIF_PK_STR += "'" + PPIF.PPIF_PK + "'";
                    }
                })
                console.log(PPIF_PK_ARR);
                myDB.T_PPI_ANSWER.get("PPIA_PPIF_PK IN ("+PPIF_PK_ARR.join()+")", function( res) {
                    var PPIA_ARR = [];
                    if(res.length === 0) {
                        reqObj.PPIANS = []
                    } else {
        ...
        function checkfinal(reqObj){ 
            ...
            $.ajax({
                ...
                cache: false,
                type: "POST",
                data: "strMode=updatePpiData&data="+JSON.stringify(reqObj),
                crossDomain: true,
                processData: false,
                success: function(result){
                    if(result=="Success"){
                        myDB.dbShell.transaction(function(tx){ //must delete this client, already synced with server
                            tx.executeSql("DELETE FROM T_PPI_FORM WHERE PPIF_CHKNEW=2"); 
                        }, function(err){
                            syncmgr.endsync(i18n.t("messages.PPISyncFail"), 'error');
                            return false;
                        }, function(suc){
                            syncmgr.synclog(i18n.t("messages.PPISyncSuccess"));
                            PSYNC.updatePpiData = true;
                            promise.resolve();
                        });
                    } else{  
	...
	this.endPushSync = function(){
	   // if(synccounter==0) syncmgr.endsync("No data to sync.");
		...
		if(synccounter === 0) {
			...
			syncmgr.endsync(i18n.t("messages.SyncSuccess"), 'success'); 
		}
	};
	...
	******************************************/
	...
	this.endsync = function(text, type){ 
        if(type !== undefined){
            swal({
                type: type,
                title: text
...
function pushData(){
    ...
    swal({
        title: i18n.t("messages.Alert"),
        text: i18n.t("messages.ConfirmPushData"),
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#80C6C7",
        confirmButtonText: i18n.t("messages.Yes"),
...
//$("#updatebutton").on("click", function(){sync.updateRequest();});
//$("#sendeval").on("click", function(){ sync.sendEval(); });
...
$("#resetprint").on("click", function(){
    ...
    function resetClientRepay(){
        ...
        var cmd1 =  "   update T_CLIENT_REPAY_SCHEDULE set CRS_TOTALPAID = '0.00' , CRS_ATTENDED = null , CRS_SMS_STATUS= null , ";
            ...
            // cmd1 += "   crs_flexi_act_1     = '0.00',   crs_flexi_act_2    = '0.00',    crs_flexi_act_3    = '0.00', ";
            // cmd1 += "   crs_flexi_act_4     = '0.00',   crs_flexi_act_5    = '0.00',    crs_flexi_act_6    = '0.00', ";
            // cmd1 += "   crs_flexi_act_7     = '0.00',   crs_flexi_act_8    = '0.00',    crs_flexi_act_9    = '0.00', ";
            // cmd1 += "   crs_flexi_act_10    = '0.00',   crs_flexi_act_11   = '0.00',    crs_flexi_act_12   = '0.00', ";
            // cmd1 += "   crs_flexi_act_13    = '0.00',   crs_flexi_act_14   = '0.00',    crs_flexi_act_15   = '0.00', ";
            // cmd1 += "   crs_flexi_act_16    = '0.00',   crs_flexi_act_17   = '0.00',    crs_flexi_act_18   = '0.00', ";
            // cmd1 += "   crs_flexi_act_19    = '0.00',   crs_flexi_act_20   = '0.00', ";//FLEXI ACT FIELDS
    ...
    function deletePrdTxn (){
        ...
        myDB.execute(cmd2, function(results){
            // alert("Print reset.");
            //alert(i18n.t("messages.PrintReset"));
            d.resolve(results);
        });
...
if(true){
    ...
    setTimeout(function(){
        ...
        var cmd =   ' SELECT CRS_FLAG, CRS_ATTENDED,CRS_DATE,CRS_PK,CRS_REF_NO,CRS_LOAN_SAVING_FLAG,'+
                ' (CAST(CRS_EXP_CAPITAL_AMT AS UNSIGNED) + CAST(CRS_EXP_PROFIT_AMT AS UNSIGNED)) as savings_total_exp, '+
                ' (CAST(CRS_ACT_CAPITAL_AMT AS UNSIGNED) + CAST(CRS_ACT_PROFIT_AMT AS UNSIGNED)) as savings_total_act '+
                ' FROM T_CLIENT_REPAY_SCHEDULE LEFT JOIN T_CLIENT ON (CRS_CLT_PK = CLT_PK) '+
                ' WHERE  CRS_FLAG ="C" AND  CRS_DATE="'+date+'" AND CRS_ATTENDED != ""';//CLT_STATUS!=8 AND
        console.log("expectedamt");
        console.log(cmd);
        myDB.execute(cmd, function(results){
            console.log("123saved");
            console.log(results);
            if(results.length > 0){
 
                for (let i = 0; i < results.length; i++) {
                    const record = results[i];
                    if (record.CRS_LOAN_SAVING_FLAG == 'L') {
                        if( record.CRS_REF_NO != 'REF'){
                            ttlsave += parseFloat(record.savings_total_exp);
                        }
                    } else {
                        ttlsave += parseFloat(record.savings_total_exp);
                    }
                    ttlsaved += parseFloat(record.savings_total_act);
                }

                updateTotal(ttldis,ttldised,ttlsaved,ttlcoled,ttwthdis,ttwthdised);
                updateTotalExp(ttlsave,ttlcol);
            }
        ...
        var cmd1 = 'SELECT CPT_TXN_AMOUNT FROM T_CLIENT_PRD_TXN WHERE CPT_FLAG="C" AND DATE(CPT_DATETIME)="'+anotherdate+'" ';
        myDB.execute(cmd1,function(results){
            console.log("liner");
            console.log(results);
            $.each(results,function(i,res){
                ////console.log(res.CPT_TXN_AMOUNT);
                ttlsaved += parseFloat(res.CPT_TXN_AMOUNT);
            });
            //updateTotal(ttldis,ttldised,ttlsaved,ttlcoled,ttwthdis,ttwthdised);
        });
        ...
        cmd =   ' SELECT CRS_FLAG, CRS_ATTENDED,CRS_DATE,CRS_PK,'+
                ' (SUM(CAST(CRS_EXP_CAPITAL_AMT AS UNSIGNED) + CAST(CRS_EXP_PROFIT_AMT AS UNSIGNED))) as savings_total_exp, '+
                ' (SUM(CAST(CRS_ACT_CAPITAL_AMT AS UNSIGNED) + CAST(CRS_ACT_PROFIT_AMT AS UNSIGNED))) as savings_total_act, '+
                ' CLL_CRF_PERCENTAGE '+
                ' FROM T_CLIENT_REPAY_SCHEDULE, T_CLIENT_LOAN '+
                ' WHERE CRS_CLL_PK = CLL_PK AND CRS_LOAN_SAVING_FLAG ="L" AND CRS_FLAG in ("R","D") AND CRS_DATE="'+date+'"';//CLT_STATUS!=8 AND

        myDB.execute(cmd, function(results){
            ////console.log(results);
            if(results.length > 0 && results[0].CRS_FLAG !== null){

                $.each(results, function(i, record){

                    var crf = (100)/100;
                    ////console.log(crf);
                    ////console.log(record.savings_total_act);
                    var disamt = record.savings_total_exp;
                    var disamted = parseFloat(record.savings_total_act);
                    if(record.CRS_FLAG=='D'){

                    }
                    ttldis += parseInt(disamt);
                    ttldised += parseInt(disamted);

                });

                // ////console.log(ttldis);
                // ////console.log(ttldised);
                // ////console.log(ttlsaved);
                // ////console.log(ttlcoled);
                // ////console.log(ttwthdis);
                // ////console.log(ttwthdised);
    ...
    // var failed_disbursement = "SELECT CLL_ACTUAL_LOAN FROM T_CLIENT_LOAN WHERE CLL_DISBURSEMENT_DATE='" + date + "' AND CLL_STATUS = 34";
    // console.log('faild disb');
    // console.log(failed_disbursement);
    // myDB.execute(failed_disbursement, function(results){
    //     console.log(results);
    //     if(results.length > 0){
    //         $.each(results, function(i, record){
    //             ttlcoled += parseInt(record.CLL_ACTUAL_LOAN);
    //         })
    ...
    // });
        ...
        var imm_wth = 0;
        $('#withdrawal').hide();
        var imm_withdrawal = "SELECT CPT_TXN_AMOUNT, CPT_EXP_AMOUNT, CPT_STATUS,CPT_DATETIME FROM T_CLIENT_PRD_TXN WHERE CPT_STATUS IN (58) and CPT_FLAG IN ('D','W') and DATE(CPT_DATETIME)='"+anotherdate+"'";
        myDB.execute(imm_withdrawal, function(results){
            if(results.length > 0){
                $.each(results, function(i, record){
                    if(record.CPT_TXN_AMOUNT === "") record.CPT_TXN_AMOUNT = 0;
                    if(record.CPT_STATUS == 58){
                        imm_wth += parseFloat(record.CPT_TXN_AMOUNT);
                    }
                });
                if(imm_wth > 0 ){
                    $('#withdrawal').hide();
                    ...
                    if(imm_wth > 0) {
                        $('#withdrawal').show();
                        $('#withdrawal').html('Immediate Withdrawal: <big>$'+imm_wth+"</big>");
                    }
                }
            }
        ...
        var withdrawal = "SELECT CPT_TXN_AMOUNT, CPT_EXP_AMOUNT, CPT_STATUS,CPT_DATETIME FROM T_CLIENT_PRD_TXN WHERE CPT_STATUS IN (47,48,58) and CPT_FLAG IN ('D','W') and DATE(CPT_DATETIME)='"+anotherdate+"'";
        myDB.execute(withdrawal, function(results){
            if(results.length > 0){
                $.each(results, function(i, record){
                    if(record.CPT_TXN_AMOUNT === "") record.CPT_TXN_AMOUNT = 0;
                    if(record.CPT_STATUS == 48 || record.CPT_STATUS == 58){
                        ttwthdised += parseFloat(record.CPT_TXN_AMOUNT);
                        ttwthdis += parseFloat(record.CPT_EXP_AMOUNT);
                    } else {
        ...
        var cmd = "SELECT PRM_PK, PRM_NAME, PRM_DESCRIPTION, PRM_DEFAULT_AMOUNT FROM T_PRODUCT_MASTER";
        myDB.execute(cmd,function(prds){
            var depo_amt = 0;
            $.each(prds, function(i,prd){ 
                if(prd.PRM_DEFAULT_AMOUNT !== '' && prd.PRM_DEFAULT_AMOUNT !== '0'){
                    depo_amt += parseInt(prd.PRM_DEFAULT_AMOUNT); 
                } 
            });

            var HRprd =   ' SELECT CPM_REPAY_PER_WEEK '+
                    ' FROM T_CLIENT '+
            ...
            myDB.execute(HRprd, function(results){
                console.log('HR');
                console.log(cmd);
                console.log(results);
                if(results.length > 0){
                    for (let p = 0; p < results.length; p++) {
                        const prd = results[p];
                        ttlcol += parseInt(prd.CPM_REPAY_PER_WEEK);
                        ttlcoled += parseInt(prd.CPM_REPAY_PER_WEEK);
                    }
                }
                updateTotal(ttldis,ttldised,ttlsaved,ttlcoled,ttwthdis,ttwthdised);
                updateTotalExp(ttlsave,ttlcol);
            ...
            var deposit =   " SELECT *, CLL_STATUS, DATE(CLT_TRANING_END_DATE) as TRAINING_END "+
                        " FROM T_CLIENT,T_CLIENT_LOAN,T_LOAN_TYPE,T_LOAN_SAVING_PRD_MAPPING "+
                        " WHERE CLT_PK = CLL_CLT_PK AND CLL_STATUS IN (57,64,67) AND CLL_LTY_PK = LTY_PK AND LTY_PK = LSM_LTY_PK "+
                        " AND DATE(CLT_TRANING_END_DATE) ='"+anotherdate+"' "+
                        " GROUP BY CLT_PK ";
            myDB.execute(deposit,function(res){
                if(res){
                    $.each(res,function(i,val){
                        ...
                        ttlcol += depo_amt;
                        if ( val.CLL_STATUS == 64 || val.CLL_STATUS == 67) {
                            ttlcoled += depo_amt;
                        }
                        

                        // if(val.LSM_PRM_OPEN_BAL.indexOf('%') == -1){
                        //     opn_bal = parseFloat(val.LSM_PRM_OPEN_BAL);
                        // } else {
                        //     var perc = val.LSM_PRM_OPEN_BAL.replace('%', '');
                        //     var opn_bal = parseFloat(val.CLL_ACTUAL_LOAN ) * parseFloat(perc) / 100;
                        // }

                        // if(val.CLL_STATUS == 26 && val.TRAINING_END == anotherdate){
                        //     ttlcoled +=  opn_bal;
                        // }
...
function updateTotal(ttldis,ttldised,ttlsaved,ttlcoled,ttwthdis,ttwthdised){
	...
	console.log("ttlsaved "+ttlsaved);
	console.log("ttlcoled "+ttlcoled);
	console.log("ttwthdised "+ttwthdised);
	console.log("ttwthdis "+ttwthdis);
	console.log("ttldis "+ttldis);
	console.log("ttldised "+ttldised);
...
async function changeIPPop() {
	const {value: ip} = await Swal.fire({
		title: 'Select IP',
		text: 'Currrent Server : ' + window.SERVERS[window.serverURL],
		input: 'select',
